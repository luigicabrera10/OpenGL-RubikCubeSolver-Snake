...
class Rubik_CubeSolver {
  public:
    Rubik_CubeSolver(const Rubik_Cube& cube): cube_(cube) {}
    virtual ~Rubik_CubeSolver() {}
  ...
  private:
    Rubik_CubeSolver(const Rubik_CubeSolver& other) {}
    Rubik_CubeSolver& operator=(const Rubik_CubeSolver& other) {}
...
class Rubik_Cube3BasicSolver: public Rubik_CubeSolver {
  public:
    Rubik_Cube3BasicSolver(const Rubik_Cube& cube):
        Rubik_CubeSolver(cube) { assert(cube_.GetDim() == 3); }
...
std::string Rubik_Cube3BasicSolver::DoSolve() {
    std::string moves;
...
// Step 1: Up Cross
bool Rubik_Cube3BasicSolver::IsUpCrossSolved() {
    return (IsCrossOriented(U) && (GetCrossMatchCount(UE) == 4));
}


inline bool Rubik_Cube3BasicSolver::IsCrossOriented(const CUBE_FACE& f) {
...
inline bool Rubik_Cube3BasicSolver::IsCornerOriented(const CUBE_FACE& f) {
    char f_char = cube_.GetMappedFaceChar(f);
...
inline int Rubik_Cube3BasicSolver::GetCrossMatchCount(const FACE_EDGE& fe) {
    int match_cnt = 0;
...
std::string Rubik_Cube3BasicSolver::SolveUpCross() {
    std::string moves;
...
// Step 2: Up Corners
bool Rubik_Cube3BasicSolver::IsUpCornersSolved() {
    const char u_face = cube_.GetMappedFaceChar(U);
...
std::string Rubik_Cube3BasicSolver::SolveUpCorners() {
    std::string moves;
    const char u_face = cube_.GetMappedFaceChar(U);
...
// Step 3: Second Layer
bool Rubik_Cube3BasicSolver::IsSecondLayerSolved() {
    for (int i = (int)L; i <= (int)B; i ++)
...
std::string Rubik_Cube3BasicSolver::SolveSecondLayer() {
...
// Step 4: Down Cross
bool Rubik_Cube3BasicSolver::IsDownCrossSolved() {
    return IsCrossOriented(D) && (GetCrossMatchCount(DE) == 4);
}


std::string Rubik_Cube3BasicSolver::SolveDownCross() {
    std::string moves;
    char d_face = cube_.GetMappedFaceChar(D);
...
// Step 5: Down Corners
inline bool Rubik_Cube3BasicSolver::IsDownCornerMatched() {
    char f_face = cube_.GetMappedFaceChar(F);
    char l_face = cube_.GetMappedFaceChar(L);
    char d_face = cube_.GetMappedFaceChar(D);
...
inline int Rubik_Cube3BasicSolver::GetDownCornerMatchCount() {
    int match_count = 0;
    for (int i = 0; i < 4; i ++) {
...
bool Rubik_Cube3BasicSolver::IsDownCornersSolved() {
    return (GetDownCornerMatchCount() == 4) && IsCornerOriented(D);
}


std::string Rubik_Cube3BasicSolver::SolveDownCorners() {
    std::string moves;
...
void Rubik_Cube3BasicSolver::FindBestCubeOrientation() {
    int max_score = 0;
