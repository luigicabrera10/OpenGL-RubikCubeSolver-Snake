...
print(kociemba.solve(')";
	...
	bool centrosCorrectos() {
		return (
			toPosition(cubes[1][1][2].getColor('F')) == 'F' &&
			toPosition(cubes[1][2][1].getColor('R')) == 'R' &&
			toPosition(cubes[1][1][0].getColor('B')) == 'B' &&
			toPosition(cubes[1][0][1].getColor('L')) == 'L' &&
			toPosition(cubes[2][1][1].getColor('U')) == 'U' &&
			toPosition(cubes[0][1][1].getColor('D')) == 'D'
		);
	}
    void solveThread(bool shuf = 0){
		...
		while(centro) {
			int intentos = 0;
			const int maxIntentos = 4; // Ajusta según sea necesario


			while (!centrosCorrectos() && intentos < maxIntentos) {

				realizarMovimiento("H");
				if (!centrosCorrectos()) {
					realizarMovimiento("V'");
				}
				if (!centrosCorrectos()) {
					realizarMovimiento("N'");
				}
				else 
					centro = false;
		...
		while(centro) {
			...
			while()
			if(toPosition(cubes[1][1][2].getColor('F')) != 'F'){
				move("H");
				while(animation)this_thread::sleep_for(chrono::milliseconds(10));
			}
			if(toPosition(cubes[1][2][1].getColor('R')) != 'R'){
				move("N");
				while(animation)this_thread::sleep_for(chrono::milliseconds(10));
			}
			if(toPosition(cubes[1][1][0].getColor('B')) != 'B'){
				move("H");
				while(animation)this_thread::sleep_for(chrono::milliseconds(10));
			}
			if(toPosition(cubes[1][0][1].getColor('L')) != 'L'){
				move("N");
				while(animation)this_thread::sleep_for(chrono::milliseconds(10));
			}
			if(toPosition(cubes[0][1][1].getColor('D')) != 'D'){
				move("H");
				while(animation)this_thread::sleep_for(chrono::milliseconds(10));
			}
			else
    ...
    void animate(){
        ...
        else if (actualAnimation == "V") {vertical1(radianXFps, last);centro = true;}
        else if (actualAnimation == "H") {horizontal(radianXFps, last);centro = true;}
		else if (actualAnimation == "N") {vertical2(radianXFps, last);centro = true;}
        ...
        else if (actualAnimation == "V'") {i_vertical1(radianXFps, last);centro = true;}
        else if (actualAnimation == "H'") {i_horizontal(radianXFps, last);centro = true;}
		else if (actualAnimation == "N'") {i_vertical2(radianXFps, last);centro = true;}
